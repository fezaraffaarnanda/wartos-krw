...
# Cache prompt aktif dari DB (per proses; serverless cold start refresh sendiri)
_PROMPT_CACHE_TTL = 300  # detik
_prompt_cache: dict = {"text": None, "version": None, "fetched_at": 0.0}

# Batas borderline (untuk antrian review manual di UI admin)
BORDERLINE_LOW  = 40
BORDERLINE_HIGH = 59
...
def get_active_prompt() -> tuple[str, str]:
    """
    Ambil (prompt_text, version) aktif dari DB dengan cache TTL.
    Fallback ke konstanta SYSTEM_PROMPT / PROMPT_VERSION jika DB gagal/kosong.
    """
    now = time.time()
    if (
        _prompt_cache["text"] is not None
        and now - _prompt_cache["fetched_at"] < _PROMPT_CACHE_TTL
    ):
        return _prompt_cache["text"], _prompt_cache["version"]
...
def invalidate_prompt_cache() -> None:
    """Paksa reload prompt dari DB pada panggilan berikutnya (dipakai setelah apply versi baru)."""
    _prompt_cache.update({"text": None, "version": None, "fetched_at": 0.0})
...
def classify_relevance(
    ...
    active_prompt, _ = get_active_prompt()
    ...
    return {
        ...
        "reason":           reason,
        "classifier_model": build_classifier_model(llm_model),
    }
