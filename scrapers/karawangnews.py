...
KARAWANGNEWS_SOURCE = "KarawangNews"
KARAWANGNEWS_BASE   = "https://www.karawangnews.com"
KARAWANGNEWS_URL    = "https://www.karawangnews.com/search?&max-results=10"
...
_KN_DOMAIN_REGEX = re.compile(r"karawangnews\.com", re.IGNORECASE)

_KN_BOILERPLATE_PREFIXES = (
    "BACA JUGA",
    "Sumber:",
    "Simak breaking news",
    "Ikuti kami",
...
_KN_BOILERPLATE_REGEX = re.compile(
    r"KarawangNews\.com\s*[–\-]+\s*",
    re.IGNORECASE,
)


def clean_content(text: str) -> str:
    text = _KN_BOILERPLATE_REGEX.sub("", text)
    text = _KN_DOMAIN_REGEX.sub("", text)

    cleaned: list[str] = []
    for raw_line in text.split("\n"):
        line = raw_line.strip()
        if any(line.startswith(prefix) for prefix in _KN_BOILERPLATE_PREFIXES):
            continue
        if line:
            cleaned.append(line)
    return "\n".join(cleaned)
...
def scrape_article(url: str) -> dict:
    ...
    author_el = soup.select_one("span[itemprop='author'] span[itemprop='name']")
    author = author_el.get_text(strip=True) if author_el else None

    date_el = soup.select_one("span.date-header-item")
    date = date_el.get_text(strip=True) if date_el else None

    tags = [t.get_text(strip=True) for t in soup.select("div#post-labels-bawah a")]

    content_el = soup.select_one("div.post-body.entry-content")
    paragraphs = []
    if content_el:
...
def scrape_karawangnews(n: int) -> list:
    url = KARAWANGNEWS_URL
    results = []
    page_number = 1
    article_count = 0
    ...
    while article_count < n:
        ...
        res = requests.get(url, headers=headers, timeout=15)
        soup = BeautifulSoup(res.text, "html.parser")
        items = soup.select("div.auto-read-more")

        for item in items:
            if article_count >= n:
                break
            link = item.select_one("h2.post-title.entry-title a")
            if link:
        ...
        next_page = soup.select_one("a.blog-pager-older-link")
...
def scrape_new_articles(
    ...
    """
    Scrape berita baru dari KarawangNews.
        ...
        for item in items:
            ...
            try:
                ...
                article = {
                    ...
                    "content": data["isi"] or "",
                    "tags":    data["tags"] or "",
                    "source":  KARAWANGNEWS_SOURCE,
                }
                new_articles.append(article)
                existing_urls.add(article_url)
                if on_progress:
                    on_progress(len(new_articles), KARAWANGNEWS_SOURCE)
            except Exception as exc:
...
if __name__ == "__main__":
    data = scrape_karawangnews(100)
    df = pd.DataFrame(data)
    df.to_excel("karawangnews.xlsx", index=False)
    print("\nData berhasil disimpan ke karawangnews.xlsx")
